...
import TravelPolicyAccordion from "./TravelPolicyAccordion";
import parse from "html-react-parser";
const TravelPolicySection = () => {
  const [open, setOpen] = useState(0);
  const url = "https://seatadda.co.in/api/travel-policy";
  const [TERMS_DATA, set_TERMS_DATA] = useState([""]);
  ...
  const fetchInfo = async () => {
    ...
    return fetch(url)
      .then((res) => res.json())
      .then((d) => {
        set_TERMS_DATA(d.date[0].content);
        setloading(false);
      });
  };
  console.log(typeof TERMS_DATA);
  useEffect(() => {
    fetchInfo();
  }, []);
  const toggle = (index) => {
  ...
  /*  const TERMS_DATA = [
        {
            ...
            desc: 'lgvhkjhc nvhvbn'
        },
        {
            title: 'Wallet Terms and Condition',
            desc: 'dfuiojhg hgfhklj vghhgjk'
        }
        , {
            title: 'Failure Terms and Condition',
            desc: 'dfuiojhg hgfhklj vghhgjk'
        }
        , {
            title: 'Cancellation Terms and Condition',
            desc: 'dfuiojhg hgfhklj vghhgjk'
        }
        , {
            title: 'Discount and Offers Terms and Condition',
            desc: 'dfuiojhg hgfhklj vghhgjk'
        }
        , {
            title: 'Refund Terms and Condition',
            desc: 'dfuiojhg hgfhklj vghhgjk'
        }
    ] */
